"""YOLO vs DNBN comparison experiment on CIFAR-10.

Steps:
1. Train YOLOv8s-cls baseline at multiple epoch counts to find its best
2. Run DNBN 8-node sweep with decreasing M/C to find minimum
3. Save all results to outputs/experiments/yolo_vs_dnbn_20260316/
"""
...
def main():
    ...
    # --- YOLOv8s-cls baseline sweep: epochs and LR ---
    configs = [
        (10, 0.001),
        (20, 0.001),
        (30, 0.001),
    ...
    for epochs, lr in configs:
        tag = f"yolov8s_e{epochs}_lr{lr}"
        print("\n" + "=" * 72)
        print(f"Training YOLOv8s-cls | epochs={epochs} lr={lr}")
        print("=" * 72)

        model = build_baseline("yolov8s", output_dim)
    ...
    # Write aggregate CSV
    agg_path = output_dir / "yolo_baseline_sweep.csv"
    ...
    print(f"\n{'='*72}")
    print(f"Best YOLO config: {best['tag']}")
